...
import { X, Lock, Clock, Zap } from 'lucide-react';
...
export default function TrialBanner({ onUpgrade }: TrialBannerProps) {
  ...
  const { planId, loading, trialActive, trialDaysLeft: daysLeft } = usePlanContext();
  ...
  if (trialActive) {
    return (
      <div className="bg-gradient-to-r from-blue-600 to-cyan-600 text-white px-4 py-2.5 flex items-center justify-between gap-4">
        <div className="flex items-center gap-2 text-sm font-medium">
          <Zap className="w-4 h-4 flex-shrink-0" />
          Free trial active — {daysLeft} {daysLeft === 1 ? 'day' : 'days'} left. All features unlocked.
        </div>
  ...
  return (
    <div className="bg-gradient-to-r from-amber-600 to-orange-600 text-white px-4 py-2.5 flex items-center justify-between gap-4">
      <div className="flex items-center gap-2 text-sm font-medium">
        ...
        Your free trial has ended. Request activation to unlock all features.
      </div>
      <div className="flex items-center gap-3 flex-shrink-0">
        <button
          onClick={onUpgrade}
