...
export default function PaywallGate({ feature, featureLabel, children, hardBlock = false }: PaywallGateProps) {
  ...
  const hasAccess = canAccessAppFeature(feature, planId) || trialActive;
  ...
  if (hardBlock) {
    return (
      <>
        ...
        <SubscriptionModal isOpen={showModal} onClose={() => setShowModal(false)} featureBlocked={featureLabel} />
      </>
    );
  }
